...
def _na(df, col): return col in df.columns and not df[col].dropna().empty

def _grid_fig(fig, title: str):
    fig.update_layout(
        template="plotly_dark", height=360, margin=dict(l=30,r=20,t=50,b=30),
...
# 1
def q1_incident_types(df: pd.DataFrame):
    figs=[]; wc=None
    if _na(df,"incident_type"):
        ...
        if _na(df,"severity_norm"):
            ...
            figs.append(_grid_fig(px.bar(t, x="incident_type", y="count", color="severity_norm", barmode="stack"),
...
# 2
def q2_client_groups(df: pd.DataFrame):
    figs=[]
    if _na(df,"client_name"):
        ...
        if _na(df,"ndis_id"):
            ...
            figs.append(_grid_fig(px.scatter(rate, x="ndis_id", y="count", color="client_name"),
                                  "Rate by NDIS ID"))
    if _na(df,"recurrence") and _na(df,"client_name"):
        bx = _grid_fig(px.box(df.dropna(subset=["recurrence"]), x="client_name", y="recurrence"),
                       "Recurrence by client")
        figs.append(bx)
    if _na(df,"age_group") and _na(df,"incident_type"):
        ct = df.groupby(["age_group","incident_type"]).size().reset_index(name="count")
        figs.append(_grid_fig(px.density_heatmap(ct, x="age_group", y="incident_type", z="count", nbinsx=6),
                              "Age group × incident type (heatmap)"))
    return figs

# 3
def q3_when(df: pd.DataFrame):
    figs=[]
    if _na(df,"incident_hour"):
        ...
        if _na(df,"severity_norm"):
            ...
            figs.append(_grid_fig(px.density_heatmap(t, x="incident_hour", y="severity_norm"),
                                  "Severity over time of day"))
    if _na(df,"dow"):
        s = df["dow"].astype(str).value_counts().reindex(
            ["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"]).fillna(0).reset_index()
        s.columns=["dow","count"]
...
def q4_resolution(df):
    ...
    Shows histogram + boxplot using Plotly.
    """
    ...
    # Tabs for histogram and boxplot
    ...
    with tab1:
        ...
        st.plotly_chart(fig1, use_container_width=True, key="resolution_hist")
        figs.append(fig1)
...
# 5
def q5_org_rates(df: pd.DataFrame):
    figs=[]
    if _na(df,"organization"):
        ...
        if _na(df,"severity_norm"):
            ...
            figs.append(_grid_fig(px.bar(t, x="organization", y="count", color="severity_norm", barmode="stack"),
                                  "Severity by organization"))
        if _na(df,"month"):
            tt = df.groupby(["month","organization"]).size().reset_index(name="count")
            figs.append(_grid_fig(px.line(tt, x="month", y="count", color="organization"),
                                  "Org trend over time"))
        if _na(df,"emotion_norm"):
            e = df.groupby(["organization","emotion_norm"]).size().reset_index(name="count")
            figs.append(_grid_fig(px.density_heatmap(e, x="organization", y="emotion_norm", z="count"),
                                  "Emotion by organization"))
    return figs

# 6
def q6_emotions(df: pd.DataFrame):
    figs=[]
    ...
    if _na(df,col):
        ...
        if _na(df,"incident_type"):
            ...
            figs.append(_grid_fig(px.bar(t, x="incident_type", y="count", color=col, barmode="stack"),
                                  "Emotion × incident type"))
        if _na(df,"organization"):
            e = df.groupby(["organization",col]).size().reset_index(name="count")
            figs.append(_grid_fig(px.density_heatmap(e, x="organization", y=col, z="count"),
                                  "Emotion × organization"))
        if _na(df,"month"):
            tt = df.groupby(["month",col]).size().reset_index(name="count")
            figs.append(_grid_fig(px.line(tt, x="month", y="count", color=col),
                                  "Emotion trend over time"))
    return figs

# 7
def q7_reporters(df: pd.DataFrame):
    figs=[]
    if _na(df,"reporter"):
        ...
        if _na(df,"organization"):
            ...
            figs.append(_grid_fig(px.bar(t, x="reporter", y="count", color="organization", barmode="stack"),
                                  "Reporter × organization"))
        if _na(df,"severity_norm"):
            t = df.groupby(["reporter","severity_norm"]).size().reset_index(name="count")
            figs.append(_grid_fig(px.bar(t, x="reporter", y="count", color="severity_norm", barmode="stack"),
                                  "Reporter × severity"))
    return figs

# 8
def q8_recurrence(df: pd.DataFrame):
    figs=[]
    if _na(df,"recurrence") and _na(df,"incident_type"):
        ...
        if _na(df,"severity_norm"):
            ...
            figs.append(_grid_fig(px.density_heatmap(ht, x="recurrence", y="severity_norm", z="count"),
                                  "Recurrence × severity"))
        if _na(df,"client_name"):
            c = df.groupby("client_name")["recurrence"].sum().reset_index().sort_values("recurrence", ascending=False).head(30)
...
# 9
def q9_actions(df: pd.DataFrame):
    figs=[]
    ...
    if _na(df, col):
        ...
        if _na(df,"incident_type"):
            t = df.groupby([col,"incident_type"]).size().reset_index(name="count")
            figs.append(_grid_fig(px.bar(t, x=col, y="count", color="incident_type", barmode="stack"),
                                  "Actions × incident type"))
        if _na(df,"severity_norm"):
            t = df.groupby([col,"severity_norm"]).size().reset_index(name="count")
            figs.append(_grid_fig(px.bar(t, x=col, y="count", color="severity_norm", barmode="stack"),
                                  "Actions × severity"))
...
# 10
def q10_text_patterns(df):
    """
    Visualization: Text pattern insights.
    Shows keyword frequency (bar chart) + wordcloud.
    """
    if "description" not in df.columns:
        st.warning("No description text found for analysis.")
        return []
    ...
    fig1 = px.bar(
        ...
        x="word",
        y="freq",
        title="Keyword frequency (top 30)",
